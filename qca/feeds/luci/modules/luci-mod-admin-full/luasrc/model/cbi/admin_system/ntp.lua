...
m = Map("system", translate("NTP time server"), translate("After the wireless gateway is powered off, the date and time settings are not saved. Enable the Sync from network function to keep the date and time current."))
m:chain("luci")


s = m:section(TypedSection, "system", translate("Synchronize browser time"))
s.anonymous = true
s.addremove = false
...
if has_ntpd then
	...
	if not has_section then
		...
		o = s:option(DynamicList, "server", translate("NTP server"))
		o.datatype = "host"
		o:depends("enable", "1")
