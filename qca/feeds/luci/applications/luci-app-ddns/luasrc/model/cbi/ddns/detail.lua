...
err_ipv6_other = bold_on ..
			...
			[[<a href="]] ..
					DISP.build_url("admin", "application", "ddns", "detail", section) ..
...
m.title = [[</a><a href="]] .. DISP.build_url("admin", "application", "ddns") .. [[">]] ..
...
m.redirect = DISP.build_url("admin", "application", "ddns")
...
if fd4 then
	...
	repeat
		...
		if s then application4[#application4+1] = s end
	until not ln
	fd4:close()
end

for _, v in UTIL.vspairs(application4) do svc4:value(v) end
...
if fd6 then
	...
	repeat
		ln = fd6:read("*l")
		local s = ln and ln:match('^%s*"([^"]+)"')
		if s then application6[#application6+1] = s end
	until not ln
	fd6:close()
end

for _, v in UTIL.vspairs(application6) do svc6:value(v) end
